...
function setNeverBlur(enabled){
  ...
  els.neverBlurCheckbox.checked = enabled;
...
export function initRevealModal(){
  ...
  // Turning it on needs a confirmation (it's the one setting that skips the warning
  // entirely, on every title, no per-image asking), turning it back off doesn't, undoing
  // a safety default isn't something that needs to be defended against.
  els.neverBlurCheckbox.checked = neverBlurIsEnabled();
  els.revealPrefCheckbox.disabled = neverBlurIsEnabled();
  els.neverBlurCheckbox.addEventListener('change', () => {
    if(els.neverBlurCheckbox.checked){
      els.neverBlurCheckbox.checked = false; // stays off until actually confirmed
